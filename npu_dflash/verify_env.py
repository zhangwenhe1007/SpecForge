...
# ---- Ascend NPU compatibility patch ----
# Some CUDA-oriented packages expect get_device_capability()
# to return a tuple like (major, minor). On torch_npu this may return None.
try:
    import torch

    def _fake_npu_device_capability(*args, **kwargs):
        return (8, 0)

    if hasattr(torch, "npu"):
        torch.npu.get_device_capability = _fake_npu_device_capability
    if hasattr(torch, "cuda"):
        torch.cuda.get_device_capability = _fake_npu_device_capability
except Exception:
    pass
# ---- end Ascend NPU compatibility patch ----
